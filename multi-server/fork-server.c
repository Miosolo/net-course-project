...
// scan the user's input, if entered "exit", shut down the server
void *exit_ctrl(void *ptr) {
	char buf[5];
...
int main(int argc, char *argv[]) {
	...
	// master proc will create an "exit" listening thread
	if (pthread_create(&listen_tid, NULL, exit_ctrl, NULL)) {
		// if failed
		perror("creating instruction listening thread");
		return 1;
	}

	while ((conn_sockfd = accept(listen_sockfd, (struct sockaddr *)&remote_addr,
															 &addr_len)) != FAIL) {
		...
		switch (pid = fork()) {  // fork the process
			case FAIL:             // error, child process not created
				perror("forking");
				close(conn_sockfd);  // abort the connection
				break;
			case 0:                  // child process:
				close(listen_sockfd);  // unbind the listening socket
				...
				while ((len = recv(conn_sockfd, buffer, BUF_SIZE, 0)) != FAIL) {
					...
					if (strncmp(buffer, "exit", len) == 0) {  // detect 'exit'
						printf("process %d closed by client\n", getpid());
						close(conn_sockfd);
						close(listen_sockfd);
						exit(0);
					}
					...
					printf("received: %s\n", buffer);
					// simply echo, including the end \0
					if (send(conn_sockfd, buffer, strlen(buffer) + 1, 0) == FAIL) {
						// waiting for user input & check send status
						// reply to server addr, if encounter an error:
						perror("replying");
						exit(1);
					}
				}       // end of while, child process will never jump out
			default:  // master process
				printf("forked process %d to handle\n", pid);
		}
	}  // end of while
}